...
struct LiquidSwipeView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if self.pageIndex != Config.colors.count - 1 {
    ...
    func button(data: SliderData) -> some View {
        let aw = (data.side == .left ? 1 : -1) * Config.arrowWidth / 3
        let ah = Config.arrowHeight / 3
        return ZStack {
            //ドラッグする円と矢印を記述
            circle(radius: Config.buttonRadius).stroke(Color.white).opacity(0.2)
            polyline(aw, ah, -aw, 0, aw, -ah).stroke(Color.white, lineWidth: 3)
        }
        .offset(data.buttonOffset)
        .opacity(data.buttonOpacity - 0.35)
    }
